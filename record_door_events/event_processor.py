...
def wait_for_segments(end_time):
    wait_until = end_time + max(3.0, SEGMENT_SECONDS * 2)

    while time.time() < wait_until:
        segments = get_segments()
        if segments and segments[-1][0] >= end_time:
            return segments
        time.sleep(1)
...
def validate_video(path):
    ...
    try:
        ...
        log.info("Validated: %.2fs, %.1f MB", duration, size)
        return duration > 0
    except Exception as e:
...
def process_event(event_file):
    ...
    segments = wait_for_segments(end_time)
    ...
    try:
        ...
        if not validate_video(output_file):
            log.error("Video validation failed for output: %s", output_file.name)
            try:
                output_file.unlink()
            except FileNotFoundError:
                pass
            ...
            try:
                ...
                for event_file in event_files:
                    if event_file.is_file():
                        executor.submit(process_event, event_file)
            except Exception as e:
